fetch('data/members.json')
    ...
    .then(data => {        
        const companies = data.filter(item => item['membership level'] >= 2);

        const getRandomCompanies = (array, num) => {
            const result = [];
...
function populateGrid(companies) {
    const gridContainer = document.getElementById('spotlight');
